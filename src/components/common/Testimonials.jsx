...
export default function Testimonials({ reviews = [] }) {
  ...
  // Limit to only two unique testimonials (no repetition)
  ...
  return (
    <div className="w-full">
      ...
      {isSimple ? (
        // Simple 2-card grid (no scroll, no arrows)
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8 px-6 sm:px-8 lg:px-12">
          {normalizedExtended.map((r) => (
            <div
              key={r.id}
              className={[
                "w-full min-h-[380px] sm:min-h-[420px] lg:min-h-[450px]",
                "p-6 sm:p-8 lg:p-10 flex flex-col justify-between relative",
            ...
            >
              <div className="absolute top-4 right-4 sm:top-6 sm:right-6">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center bg-white/10 text-white/60">
                  <i className="fa-solid fa-quote-right text-sm sm:text-base" />
                </div>
              </div>

              <div className="flex-1 flex flex-col justify-start pt-8 sm:pt-10">
                <p className="text-sm sm:text-base lg:text-lg leading-relaxed text-left whitespace-pre-line font-light tracking-wide text-white/90 dark:text-white/90 light:text-gray-700">
                  "{r.testimonial}"
                </p>
              </div>

              <div className="mt-8 sm:mt-10 text-center">
                <div className="font-semibold text-base sm:text-lg mb-1 text-white dark:text-white light:text-gray-800">
                  {r.name}
                </div>
