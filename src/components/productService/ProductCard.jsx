...
export default function ProductCard({ header, description, buttonText, image, onButtonClick }) {
  return (
    <div className="grid md:grid-cols-2 gap-10 items-center text-secondary dark:text-primary rounded-2xl p-6 border border-transparent transition-all duration-300 cursor-pointer hover:-translate-y-2 hover:shadow-brandLg hover:!border-[#12d576] active:scale-[0.98]">
      ...
      <div>
        ...
        {buttonText && (
          <div className="mt-6">
            <button onClick={onButtonClick} className="inline-flex items-center rounded-full bg-tertiary px-5 py-2 text-sm font-semibold text-secondary transition-all duration-300 hover:-translate-y-0.5 hover:shadow-brand">
              {buttonText}
            </button>
          </div>
        )}
